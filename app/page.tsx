  ...
  return (
    <>
      ...
      <main id="top">
        ...
        <section className="search-section section-shell" id="search"><div className="section-heading"><div><p className="eyebrow dark-eyebrow">Make every purchase count</p><h2>Before you buy,<br /><span>check the trace.</span></h2></div><p>Buying second-hand? Search our registry using a bike&apos;s serial number and make a confident choice.</p></div><form className="search-box" onSubmit={search}><label htmlFor="serial-search">Search by serial number, brand or model</label><div className="search-row"><input id="serial-search" type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="e.g. WSBC6019 or Trek Marlin" /><button className="button button-green" type="submit">Search registry <FiArrowRight /></button></div><p className="search-result" aria-live="polite">{result}</p></form></section>
