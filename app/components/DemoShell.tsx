...
export function DemoShell({ children, active = 'Overview', showSidebar = false }: { children: React.ReactNode; active?: string; showSidebar?: boolean }) {
  ...
  return <div className={`app-frame${showSidebar ? '' : ' public-frame'}`}>
    {showSidebar && <aside className="app-sidebar">
      <Link className="brand app-brand" href="/"><BrandMark /></Link>
    ...
    <div className="app-content"><header className="app-topbar"><div><Link className="public-top-brand" href="/"><BrandMark /></Link><span className="mobile-app-brand"><BrandMark /></span><span className="breadcrumb">Workspace / {active}</span></div><div className="topbar-actions">{showSidebar ? <><Link href="/search" className="topbar-search">⌕ <span>Search registry</span></Link><span className="notification">♢<i /></span><Link className="button button-dark button-small" href="/register">Add a bike <span>+</span></Link></> : <nav className="public-top-nav"><Link href="/search">Search a bike</Link><Link href="/organizations">For organisations</Link><Link href="/login">Log in</Link><Link className="button button-dark button-small" href="/register">Register a bike <span>↗</span></Link></nav>}</div></header><main className="app-main">{children}</main></div>
  </div>
}
